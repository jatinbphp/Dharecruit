...
class BDMSubmissionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $submissions = $this->getQuery($request);
            return $this->getSubmissionHtml($submissions, $request);
        }
    ...
    public function teamLeadSubmissions(Request $request)
    {
        if(!isLeadUser()){
            return redirect('/');
        }

        $data['menu'] = "Team Submission";
        $data['type'] = 3;
        if ($request->ajax()) {
    ...
    public function getQuery($request)
    {
        ...
        if($user->role == 'recruiter'){
            if($request->user_type == 'lead_user' && isLeadUser()){
                if(getTeamMembers()){
                    if($request->team_users){
                        $submissions->whereIn('submissions.user_id', $request->team_users);
                    } else {
                        $submissions->whereIn('submissions.user_id', getTeamMembers());
                    }
                } else {
                    $submissions->where('submissions.user_id', 0);
                }
            } else {
            ...
            if($request->user_type == 'lead_user' && isLeadUser()){
                if(getTeamMembers()){
                    if($request->team_users){
                        $loggedinBdmrequirementIds = Requirement::whereIn('user_id', $request->team_users)->pluck('id')->toArray();
                    } else {
                        $loggedinBdmrequirementIds = Requirement::whereIn('user_id', getTeamMembers())->pluck('id')->toArray();
                    }
                } else {
                    $loggedinBdmrequirementIds = [];
                }
            }else {
    ...
    public function getSubmissionHtml($query, $request)
    {
        return Datatables::of($query)
            ->addColumn('candidate_name', function($row) use(&$request){
                ...
                return ($candidateCount ? "<span class='badge bg-indigo position-absolute top-0 start-100 translate-middle'>$candidateCount</span>" : "")
                    ...
                    '<div  class="a-center pt-2 pl-2 pb-2 pr-2 '. $candidateCss.'" style="width: fit-content;">
                            <span class="'.$candidateClass.' '. ($request->user_type != "lead_user" ? "candidate" : "") .' candidate-'.$row->id.'" style="'.$candidateBorderCss.'" data-cid="'.$row->id.'">'.($isSamePvCandidate ? "<i class='fa fa-info'></i>  ": ""). $candidateName. '-' .$row->candidate_id. '</span>
                        </div>
