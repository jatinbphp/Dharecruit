                ...
                <div class="col-3 text-right">
                    <div class="row">
                        ...
                        <div class="col-2 text-right">
                            <label class="control-label mr-3 mt-1 h5" style="font-weight: 400" for="individual_submissions_count_step_size">Step:</label>
                        </div>
                        ...
                        <div class="col-2 text-right">
                            <div class="btn-group btn-group-toggle mb-2" data-toggle="buttons">
                                <label class="btn btn-sm btn-outline-danger">
                                    <input type="radio" class="individual-submissions-count-next-prev-button" data-type="-1" name="individual-requirement-assign-next-prev-options" autocomplete="off"><i class="fa fa-arrow-circle-left" data-toggle="tooltip" title="Previous" data-trigger="hover"></i>
                                </label>
                                <label class="btn btn-sm btn-outline-danger">
                                    <input type="radio" class="individual-submissions-count-next-prev-button" data-type="1" name="individual-requirement-assign-next-prev-options" autocomplete="off"><i class="fa fa-arrow-circle-right" data-toggle="tooltip" title="Next" data-trigger="hover"></i>
                                </label>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $(".individual-submission-day-type").change(function (){
            ...
            $('#individual_submissions_count_step_size').val("0").trigger("change");
            $(".individual-submissions-count-next-prev-button").prop("checked", false).parent().removeClass('active');
        ...
        $('.individual-submission-type').on('change', function() {
            $("#individual_submissions_count_step_size").trigger("change");
            prepareindividuaSubmissionCounts();
        });
        ...
        $('.individual-submissions-count-next-prev-button').click(function (){
            const stepValue = parseInt($('#individual_submissions_count_step_size').val());
            if(!stepValue){
                swal('Error', 'Please Select Step Size', 'error');
                return;
            }
            const fromDateInput = $('#individual_submission_count_fromDate');
            const toDateInput   = $('#individual_submission_count_toDate');
            const stepType      = $(".individual-submission-type:checked").attr("data-type");
            const step          = $(this).attr('data-type');
            setDateForNextPrevButtons(step, fromDateInput, toDateInput, stepValue, stepType);
            prepareindividuaSubmissionCounts();
        })

        $('#individual_submissions_count_step_size').change(function(){
            ...
            const fromDateInput = $('#individual_submission_count_fromDate');
            const toDateInput   = $('#individual_submission_count_toDate');
            $(".individual-submission-day-type").prop("checked", false).parent().removeClass('active');
            const stepType = $(".individual-submission-type:checked").attr("data-type");
