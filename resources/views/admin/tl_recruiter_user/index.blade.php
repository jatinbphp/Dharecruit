                ...
                <div class="col-12">
                    <div class="card card-info card-outline">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-12">
                                    <a href="{{ route('tl_recruiter_user.create') }}"><button class="btn btn-info float-right" type="button" style="margin-right: 1.5%;"><i class="fa fa-plus pr-1"></i> Add New</button></a>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    $(function () {
        var table = $('#usersTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('tl_recruiter_user.index') }}",
            columns: [
        ...
        $('#usersTable tbody').on('click', '.deleteUser', function (event) {
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url: "{{url('admin/tl_recruiter_user')}}/"+roleId,
                        type: "DELETE",
